...
namespace PygmyMonkey.GifCreator
{
	public class PreviewRenderer
	{
		...
		public void drawInspector()
		{
			...
			{
				defaultRenderTexture = new RenderTexture(1, 1, 0, RenderTextureFormat.ARGB32);
				defaultRenderTexture.wrapMode = TextureWrapMode.Clamp;
				defaultRenderTexture.filterMode = FilterMode.Bilinear;
				defaultRenderTexture.anisoLevel = 0;
				defaultRenderTexture.hideFlags = HideFlags.HideAndDontSave;
			}
			...
			if (GUIUtils.DrawHeader("Preview"))
			{
				using (new GUIUtils.GUIContents())
				{
					...
					using (new GUIUtils.GUIEnabled(isEnabled && (Recorder.Singleton.State == RecorderState.Paused || Recorder.Singleton.State == RecorderState.None)))
					{
						...
						{
							frameIndex = 0;
							frameArray = new ITexture[1] {
								new RTexture(defaultRenderTexture)//defaultRenderTexture
							};
						}
						...
						Texture currentRenderTexture = frameArray[frameIndex].GetTexture2D();
						if (currentRenderTexture == null || (Recorder.Singleton != null && Recorder.Singleton.State == RecorderState.Recording))
						{
							currentRenderTexture = defaultRenderTexture;
						}
						...
						using (new GUIUtils.GUIEnabled(GUI.enabled && frameArray.Length > 1))
						{
							...
							using (new GUIUtils.GUIHorizontal(GUILayout.Width(spaceNeeded)))
							{
								...
								using (new GUIUtils.GUIEnabled(frameIndex > frameStart))
								{
							...
							EditorGUILayout.LabelField("Frame interval selected", (int)frameStart + " to " + (int)frameEnd + " (" + (frameEnd - frameStart + 1) + " frames)");
						}
					}
				}
			}
