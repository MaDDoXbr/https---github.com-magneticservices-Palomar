...
---------------------------------------- Camera section -----------------------------------------
The very first thing you need to do, is drag and drop your camera to the camera field.
Gif Creator will get whatever this particular camera render to create your gif.

There is actually two methods to create your gifs.
- The fastest method:
As we said, you just specify a single camera, and it will record each frame coming from what
this exact camera sees.
...
a method that is slow... that is getting each frame, the entire screen pixel array.

In order to use the slowest method, you just need to toggle the 'Record entire Game View' toggle
right under the camera field.

If you want to create a gif from a high consuming frame rate project you have, the slowest method
might not work really well, and will greatly impact your frame rate. But if you have a small
project, like a 2D game, or a game where you don't really need to have a high frame rate, it can
work really great :)
...
record your red preview rectangle when making the gif :) But even in play mode you can toggle it
whenever you want to see it and adjust it.

--- Frames Per Seconds ---
You can define here, how many frames per seconds you want to record.
Of course, the more frame you have, the bigger the gif will be, so adjust according to your needs :)
...
reduce that to a smaller size so that the gif is not to big and can be easily viewed by everyone.
...
Here, you can choose to record a specific part of your gif file and not the entire frames.
...
--- Frames ---
Here, you can use this min/max slider to define the start and end you want to save.
...
--- Repeat ---
Base on the repeat value, you can decide if :
...
GIF specification). Lower values (minimum = 1) produce better colors, but slow processing
significantly. Higher values will speed up the quantization pass at the cost of lower image
quality (maximum = 100).

--- Output width ---
You can define the final output width of the gif file. The best thing to do, is keep this width
to same record width you defined earlier in the parameters sections.
...
--- Upload to imgur.com ---
Pressing this button, will first save your gif to a tmp folder and then start the upload.
...
Mac: /Users/USERNAME/Library/Application Support/PygmyMonkey/GifCreator/
Windows: C:\Users\USERNAME\AppData\LocalLow\PygmyMonkey\GifCreator\)
